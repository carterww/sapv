...
std::unique_ptr<std::string> CurlHttpClient::get(const std::string_view url,
    ...
    if (!accepted_encodings.empty()) {
        ...
        std::unique_ptr<std::vector<std::string>> encodings = utils::string::split(accepted_encodings, ", ");
        // Set encodings. Can I do multiple encodings?
        for (int i = 0; i < encodings->size(); i++) {
            curl_easy_setopt(this->curl, CURLOPT_ACCEPT_ENCODING, encodings->at(i).data());
        }
    }
    ...
    curl_easy_setopt(this->curl, CURLOPT_WRITEFUNCTION, curl_write_function);
    ...
    std::unique_ptr<std::string> result = std::make_unique<std::string>(buffer.data);
    free(buffer.data);
